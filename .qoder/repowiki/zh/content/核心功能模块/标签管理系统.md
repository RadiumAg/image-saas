...
- [tag-file-list.tsx](file://src/app/dashboard/apps/[appId]/tag-file-list.tsx)
...
- 修正了服务器端数据库查询中的表引用错误，确保应用作用域的正确性
- 优化了标签文件列表组件的缓存行为，提供更及时的数据刷新
...
**更新** 本次更新重点集成了完整的通知系统（Sonner），为用户提供实时的操作反馈和状态提示。同时修正了数据库查询中的表引用错误，确保标签查询的正确性和应用作用域的严格隔离。**重要更新** CategoryType 类型现已扩展为灵活的字符串类型，支持任意分类名称，移除了原有的 ('person' | 'location' | 'event') 硬编码限制。**新增** 应用作用域标签支持，通过 appId 字段实现应用级别的标签隔离和管理。**新增** 标签文件列表组件的缓存优化，通过设置 staleTime 和 gcTime 为 0，确保数据的即时刷新和一致性。
...
UI_TagFileList["TagFileList 组件<br/>标签文件列表与缓存优化"]
end
subgraph "服务层"
...
- [tag-file-list.tsx:50-58](file://src/app/dashboard/apps/[appId]/tag-file-list.tsx#L50-L58)
...
- **更新** 标签文件列表组件缓存优化
  - 设置 staleTime: 0 和 gcTime: 0，确保数据即时过期和回收
  - 优化无限滚动查询的缓存行为，提供更及时的数据刷新
- 前端标签组件
...
## 架构总览
标签系统采用 tRPC 作为前后端通信桥梁，服务端路由集中于 tagsRouter 和 appsRouter，数据库通过 Drizzle ORM 访问。AI 识别模块封装在服务端，前端仅负责触发与展示结果。**更新** 新增应用特定标签集成，AI识别时会传入当前应用的所有标签，提供更精准的分类。**重要更新** CategoryType 现在支持任意字符串，不再受硬编码限制。**新增** 应用作用域标签支持，通过 appId 字段实现应用级别的标签隔离。**新增** 通知系统集成，为用户提供实时的操作反馈和状态提示。**更新** 标签文件列表组件优化了缓存策略，通过设置 staleTime 和 gcTime 为 0，确保数据的即时刷新和一致性。
...
- **更新** 统一的 toast 调用：在标签管理页面中，所有 CRUD 操作都集成了实时通知反馈
...
- **更新** 统一的 CRUD 操作反馈：所有标签操作都集成了实时的 toast 通知
...
### 标签文件列表组件缓存优化
- **新增** 缓存策略优化：设置 staleTime: 0 和 gcTime: 0，确保数据即时过期和回收
- **新增** 即时数据刷新：通过设置 refetchOnMount: true，确保组件挂载时总是获取最新数据
- **新增** 无限滚动查询优化：在标签文件列表中，通过设置 staleTime 和 gcTime 为 0，提供更及时的数据刷新
- **新增** 上传后数据更新：在文件上传完成后，通过 setInfiniteData 方法直接更新缓存数据，避免等待重新获取

```mermaid
sequenceDiagram
participant Component as "TagFileList 组件"
participant Cache as "缓存系统"
participant Query as "无限查询"
Component->>Cache : 设置 staleTime : 0, gcTime : 0
Component->>Query : 启用查询 (refetchOnMount : true)
Query->>Cache : 数据即时过期
Cache-->>Query : 返回最新数据
Component->>Cache : setInfiniteData 更新缓存
Cache-->>Component : 即时响应更新
```

**图表来源**
- [tag-file-list.tsx:50-58](file://src/app/dashboard/apps/[appId]/tag-file-list.tsx#L50-L58)
- [tag-file-list.tsx:164-182](file://src/app/dashboard/apps/[appId]/tag-file-list.tsx#L164-L182)
...
- 查询优化
  ...
  - **更新** 缓存优化：标签文件列表组件设置 staleTime 和 gcTime 为 0，确保数据即时刷新
- 事务与幂等
...
- 前端优化
  ...
  - **更新** 标签文件列表缓存优化：通过设置 staleTime 和 gcTime 为 0，提供更及时的数据刷新
...
- **新增** 标签文件列表缓存问题
  - 现象：标签文件列表数据不及时刷新
  - 处理：确认 staleTime 和 gcTime 已设置为 0，检查 refetchOnMount 配置
  - **新增** 检查无限滚动查询：确认 getNextPageParam 正确设置，避免缓存导致的数据陈旧
...
- **新增** 标签文件列表缓存优化
  - 设置 staleTime: 0 和 gcTime: 0，确保数据即时过期和回收
  - 优化无限滚动查询的缓存行为，提供更及时的数据刷新
  - 通过 setInfiniteData 直接更新缓存数据，避免等待重新获取
- 环境变量
