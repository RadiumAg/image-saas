...
- **更新** 应用作用域标签支持，新增 appId 字段实现应用级别的标签隔离
...
**更新** 本次更新重点增强了AI图像标签识别系统，实现了应用特定标签集成、智能分类和动态提示生成功能，显著提升了标签识别的准确性和智能化水平。**重要更新** CategoryType 类型现已扩展为灵活的字符串类型，支持任意分类名称，移除了原有的 ('person' | 'location' | 'event') 硬编码限制。**新增** 应用作用域标签支持，通过 appId 字段实现应用级别的标签隔离和管理。
...
## 架构总览
标签系统采用 tRPC 作为前后端通信桥梁，服务端路由集中于 tagsRouter 和 appsRouter，数据库通过 Drizzle ORM 访问。AI 识别模块封装在服务端，前端仅负责触发与展示结果。**更新** 新增应用特定标签集成，AI识别时会传入当前应用的所有标签，提供更精准的分类。**重要更新** CategoryType 现在支持任意字符串，不再受硬编码限制。**新增** 应用作用域标签支持，通过 appId 字段实现应用级别的标签隔离。
